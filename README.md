...
**Робоча адреса: https://tnzkv.github.io/architectural-render-qc/**

Відкривати саме її, а не локальний файл. Причина не в зручності, а в збереженні даних:
браузер **ніколи** не видає режим «постійне сховище» сторінкам, відкритим як `file://`,
тому історія перевірок і всі оцінки TP/FP/FN там завжди лишаються витісненими — браузер
має право стерти їх під тиском місця на диску. На `https://` походження це вже звичайне
сховище сайту.

Дані нікуди не публікуються: на GitHub Pages лежить **лише код**. Історія, зображення й
API-ключ живуть у сховищі вашого браузера й нікуди звідти не йдуть, окрім самих запитів
до `api.anthropic.com` під час аналізу.

**[diagnostics.html](diagnostics.html)** (`…github.io/architectural-render-qc/diagnostics.html`)
— сторінка тільки для читання: показує, що саме лежить у сховищі, скільки місця зайнято,
чи є дзеркальна копія історії, і дозволяє вивантажити стан у файл. Нічого не змінює.

## Швидкий перегляд — без встановлення нічого

**[index.html](index.html)** — та сама логіка (upload, review з TP/FP/FN, метрики, історія, CSV-експорт), але одним файлом на чистому JS без збірки чи бекенда. Дані живуть у `localStorage` браузера, мок-порівняння — той самий алгоритм, що й у `backend/app/services/mock_ai.py`, портований у JS.

Інтерфейс — один екран: зліва компактна бічна панель для завантаження рендера й креслення та кнопка «Перевірити», справа — історія всіх перевірок (клік по рядку відкриває review). Окремої сторінки проєктів немає — один спільний робочий простір.

Просто відкрийте `index.html` у браузері (подвійний клік або перетягнути у вкладку) — якщо ще немає жодної перевірки, з'явиться кнопка «✨ Завантажити демо-дані» з двома тестовими зображеннями, щоб одразу пройти весь workflow.
...
### Реальний AI-аналіз (опційно)

Кнопка **«⚙ AI-налаштування»** дозволяє додати Anthropic API-ключ (зберігається лише в `localStorage` браузера, ніколи не в git). З ключем аналіз реально йде через **Claude Opus 4.8 Vision** зі structured output — system-промпт явно забороняє перевіряти розміри/масштаб, ігнорує кружечки-виноски й розмірні лінії на кресленнях (це посилання на будівельні специфікації, а не архітектурні елементи), і враховує, що секції багатоквартирних/twin-home фасадів можуть бути показані в іншому порядку між кресленням і рендером. Без ключа — мок-дані.

### Імпорт з PDF (елевації з багатосторінкових комплектів)

Кнопка «+ Завантажити» на кресленнях приймає `.pdf` — відкривається вбудований переглядач сторінок (pdf.js, повністю в браузері), де можна погортати весь комплект креслень і вибрати саме потрібну елевацію; вона рендериться в PNG і зберігається як звичайний asset-креслення. Пряме читання бінарного `.dwg` в браузері неможливе без стороннього CAD-конвертера — при спробі завантажити `.dwg` застосунок підкаже експортувати елевацію в PDF/PNG (наприклад через безкоштовний ODA File Converter) і завантажити результат.
